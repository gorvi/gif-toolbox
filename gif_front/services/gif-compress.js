    ...
    fs.readFile({
      ...
      success: (res) => {
        try {
          ...
          reject(new Error((e && e.message) ? e.message : '读取文件失败'))
        }
      },
...
function assertGifBytes(bytes) {
  ...
  if (info.kind === 'webp') {
    throw new Error('当前文件是 WebP 动图（微信常见），暂不支持压缩；请尝试“聊天选择”并选择 .gif 文件')
  }
...
async function getGifMeta(inputPath) {
  ...
  for (let i = 0; i < frames; i++) {
    ...
    const d = Number(info && info.delay)
    durationCs += Math.max(1, Number.isFinite(d) ? d : 1)
  }
  const fps = durationCs > 0 ? (frames * 100) / durationCs : 0
  return {
    width: reader.width,
...
async function compressGif(options) {
  const { inputPath, maxSidePx, frameStep, dither, onProgress } = options || {}
  if (!inputPath) throw new Error('请先选择 GIF')
  ...
  const fs = wx.getFileSystemManager()
  const outPath = `${wx.env.USER_DATA_PATH}/gif_compress_${Date.now()}.gif`
