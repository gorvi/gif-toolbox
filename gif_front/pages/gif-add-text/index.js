...
const { addTextToGif } = require('../../services/gif-add-text')
...
Page({
  data: {
    ...
    text: '',
    positionIndex: 2,
    positionLabels: POSITION_OPTIONS.map((x) => x.label),
    sizeIndex: 1,
    sizeLabels: SIZE_OPTIONS.map((x) => x.label),
    colorIndex: 0,
    colorLabels: COLOR_OPTIONS.map((x) => x.label),
    strokeEnabled: true,
    bgEnabled: false,
  ...
  computePrimaryActionText() {
    ...
    if (!this.data.outPath) return '开始加字'
    if (this.data.outputDirty) return '重新加字'
    return '预览'
  },
  ...
  onTextInput(e) {
    ...
    this.setData({ text: value }, () => this.markOutputDirty())
  },

  onPositionPick(e) {
    if (this.data.processing) return
    const idx = Number(e.detail && e.detail.value) || 0
    this.setData({ positionIndex: idx }, () => this.markOutputDirty())
  },

  onSizePick(e) {
    if (this.data.processing) return
    const idx = Number(e.detail && e.detail.value) || 0
    this.setData({ sizeIndex: idx }, () => this.markOutputDirty())
  },

  onColorPick(e) {
    if (this.data.processing) return
    const idx = Number(e.detail && e.detail.value) || 0
    this.setData({ colorIndex: idx }, () => this.markOutputDirty())
  },

  onStrokeChange(e) {
    if (this.data.processing) return
    const checked = !!(e.detail && e.detail.value)
    this.setData({ strokeEnabled: checked }, () => this.markOutputDirty())
  },

  onBgChange(e) {
    if (this.data.processing) return
    const checked = !!(e.detail && e.detail.value)
    this.setData({ bgEnabled: checked }, () => this.markOutputDirty())
  },
  ...
  async onGenerate() {
    ...
    const position = (POSITION_OPTIONS[this.data.positionIndex] && POSITION_OPTIONS[this.data.positionIndex].value) || 'BOTTOM'
    const sizeMode = (SIZE_OPTIONS[this.data.sizeIndex] && SIZE_OPTIONS[this.data.sizeIndex].value) || 'M'
    const color = (COLOR_OPTIONS[this.data.colorIndex] && COLOR_OPTIONS[this.data.colorIndex].value) || '#ffffff'
    ...
    const textConfig = {
      ...
      sizeMode,
      color,
      stroke: !!this.data.strokeEnabled,
      strokeColor: color === '#000000' ? '#ffffff' : '#000000',
      bg: !!this.data.bgEnabled,
      bgColor: '#000000',
      bgAlpha: 0.36,
    }
    ...
    try {
      const res = await addTextToGif({
        ...
        onProgress: ({ step, index, total }) => {
          ...
          if (t > 0) {
            ...
            if (step === '解码') pct = Math.round(r * 55)
            else if (step === '缩放') pct = 55 + Math.round(r * 10)
            else if (step === '加字') pct = 65 + Math.round(r * 8)
            else if (step === '量化') pct = 73 + Math.round(r * 16)
            else if (step === '编码') pct = 89 + Math.round(r * 9)
      ...
      const outSizeText = `输出：${res.width}×${res.height}px · ${res.frames}帧 · ${formatBytes(res.size)}`
      this.setData({
        outPath: res.outPath,
        outSizeText,
        outBytes: res.size || 0,
